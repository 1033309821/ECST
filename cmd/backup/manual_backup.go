...
func main() {
	...
	case "GetPooledTxs":
		fmt.Println("=== Fuzz GetPooledTxs Testing Tool ===")
		getPooledTxs(config, getPooledTxsNodeIndex)
		// Test soft limit
	case "test-soft-limit":
		fmt.Println("=== Fuzz Test All Clients Soft Limit ===")
	...
	case "test-soft-limit-report":
		fmt.Println("=== Fuzz Soft Limit Test Report ===")
		TestSoftLimitForReport(config)

	case "oneTransaction":
		// Single node testing, send only one transaction
		fmt.Println("=== Fuzz Single-Transaction Testing Tool ===")

		sendTransaction(config)
	case "largeTransactions":
		// Single node testing, send only one transaction
		fmt.Println("=== Fuzz Large-Transaction Testing Tool ===")
